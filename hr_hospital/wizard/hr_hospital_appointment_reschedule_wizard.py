...
class AppointmentRescheduleWizard(models.TransientModel):
    ...
    appointment_id = fields.Many2one('hr_hospital.appointment', string='Current Appointment', required=True)
    ...
    @api.constrains('new_appointment_date', 'new_appointment_time', 'new_doctor_id')
    def _check_availability(self):
        """Check that the new doctor is available at the new date and time."""
        for record in self:
            overlapping_appointments = self.env['hr_hospital.appointment'].search([
    ...
    def action_reschedule_appointment(self):
        """Reschedule the appointment by updating the doctor, date, and time."""
